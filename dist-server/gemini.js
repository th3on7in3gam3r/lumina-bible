                ...
                if (userId) {
                    try {
                        await pool.query(`INSERT INTO gallery (user_id, local_id, url, reference, text, date)
                             VALUES ($1, $2, $3, $4, $5, $6)
                             ON CONFLICT (user_id, local_id) DO NOTHING`, [userId, localId, rawBase64, reference, verseText, date]);
                    }
                    catch (dbErr) {
...
router.post('/search', authenticateToken, async (req, res) => {
    ...
    if (!ai)
        return res.status(503).json({ answer: "AI features are currently unavailable because the Gemini API key is not configured.", results: [] });
    ...
    try {
        ...
        if (!text)
            return res.json({ answer: "I couldn't find anything on that topic.", results: [] });
...
router.post('/ask', authenticateToken, async (req, res) => {
    ...
    if (!ai)
        return res.status(503).json({ text: "I cannot speak right now, API Key missing." });
    const systemInstruction = `
    ...
    `;
    try {
        const formattedHistory = (history || []).map((msg) => `${msg.isUser ? "User" : personaName}: ${msg.text}`).join("\n");
...
router.post('/geography', authenticateToken, async (req, res) => {
...
router.post('/deepdive', authenticateToken, async (req, res) => {
